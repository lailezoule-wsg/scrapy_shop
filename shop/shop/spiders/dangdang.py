...
class DangdangSpider(scrapy.Spider):
    ...
    async def parse(self, response):
        ...
        for index, li in enumerate(li_list, 1):
            ...
            loader.add_value("product_id",int(product_id))
        ...
        #     if engine and hasattr(engine, 'downloader'):
        #         # 获取调度器统计
        #         stats = self.crawler.stats.get_stats() # type: ignore
        #         enqueued = stats.get('scheduler/enqueued', 0)
        #         dequeued = stats.get('scheduler/dequeued', 0)
        #         self.logger.warning(f"📊 调度器: 入队 {enqueued} 个, 出队 {dequeued} 个")
                
        #         # 如果队列中有剩余请求
        #         remaining = enqueued - dequeued
        #         if remaining > 0:
        #             self.logger.warning(f"📊 剩余请求数: {remaining}")
        #         else:
        #             self.logger.warning(f"📊 调度器队列可能已空")
        # except Exception as e:
        #     self.logger.warning(f"📊 无法获取调度器状态: {e}")
